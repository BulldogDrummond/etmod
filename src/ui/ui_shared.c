...
void Script_playStream(itemDef_t *item, qboolean *bAbort, char **args) {
	const char *val=NULL;
	if (String_Parse(args, &val)) {
		//DC->stopBackgroundTrack();
		DC->startBackgroundTrack( val, "onetimeonly", 0);
	}
}
...
void Script_ExecWolfConfig(itemDef_t *item, qboolean *bAbort, char **args) {
	...
	if( Int_Parse(args, &useprofile) ) {
		...
		DC->getCVarString("cl_profile", cl_profileStr,
			sizeof(cl_profileStr));
		...
		if( useprofile && cl_profileStr[0] ) {
			...
			Com_sprintf(bakPath, sizeof(bakPath),
				"profiles/%s/%s.etmod", cl_profileStr, CONFIG_NAME);

			// tjw: restore the backup copy	generated from forcecvar
			len = trap_FS_FOpenFile(bakPath, &bak, FS_READ);
			trap_FS_FCloseFile(bak);

			if(len > 0 && !trap_FS_FOpenFile(cfgPath, &cfg, FS_WRITE)) {
				trap_FS_FOpenFile(bakPath, &bak, FS_READ);
				for(i = 0; i < len; i++) {
					byte b;
				...
				trap_FS_FCloseFile(bak);
				Com_Printf("Delete %s %d\n", bakPath, trap_FS_Delete(bakPath));
			}
...
#else
				DC->executeText(EXEC_NOW, cfgPath);
#endif // _DEBUG
			} else {
				DC->executeText(EXEC_NOW, cfgPath);
		...
		if( key == K_MOUSE1 || key == K_ENTER || key == K_MOUSE2 || key == K_MOUSE3 
			...
			if( !(item->cvarFlags & CVAR_NOTOGGLE) ) {
				if( item->type == ITEM_TYPE_TRICHECKBOX ) {
					int curvalue = DC->getCVarValue( item->cvar ); 
					switch( key ) {
					case K_MWHEELDOWN:
						if( curvalue <= 0 )
							return qfalse;
...
static void Scroll_Slider_ThumbFunc(void *p) {
	...
	// kw: hack for INTSLIDER
	if(editDef->range == 1.f) {
	...
	if (item->window.flags & WINDOW_HASFOCUS && item->cvar && Rect_ContainsPoint(&item->window.rect, DC->cursorx, DC->cursory)) {
		if (key == K_MOUSE1 || key == K_ENTER || key == K_MOUSE2 || key == K_MOUSE3) {
			...
			if (editDef) {
				...
				testRect.x -= value;
				//DC->Print("slider x: %f\n", testRect.x);
				testRect.w = (SLIDER_WIDTH + (float)SLIDER_THUMB_WIDTH);
				//DC->Print("slider w: %f\n", testRect.w);
				if (Rect_ContainsPoint(&testRect, DC->cursorx, DC->cursory)) {
					...
					//KW 25-08-2005: do a range check
					if(value < editDef->minVal)
						value = editDef->minVal;
			...
			// kw: mouse scroll function added to the scrollbar
			editFieldDef_t *editDef = item->typeData;
			if (editDef) {
				work = (editDef->maxVal - editDef->minVal) / 20;
				value = DC->getCVarValue( item->cvar );
				if( key == K_MWHEELDOWN ) {
					if( value - work >= editDef->minVal ) 
						value -= work;
					else 
						value = editDef->minVal;
				} 
				if( key == K_MWHEELUP )  { 
					if( value + work <= editDef->maxVal ) 
						value += work;
					else 
						value = editDef->maxVal;
				}
...
char* BindingFromName(const char *cvar) {
	...
	if( b1 != -1 ) {
		...
		if(b2 != -1) {
			...
			Q_strcat( g_nameBind1, sizeof(g_nameBind1), DC->translateString( " or " ) );
			Q_strcat( g_nameBind1, sizeof(g_nameBind1), g_nameBind2 );
		}
	} else {
		// kw: show [bindname] instead of (???)
		Q_strncpyz( g_nameBind1, va("[%s]", cvar), sizeof(g_nameBind1) );
	}
...
qboolean Item_Bind_HandleKey(itemDef_t *item, int key, qboolean down) {
	...
	else
	{
		...
		switch (key)
		{
			...
			case K_BACKSPACE:
				...
				if (id != -1) {
					// kw: fixed backspace now properly clears the bind
					DC->setBinding( g_bindings[id].bind1, "" );
					DC->setBinding( g_bindings[id].bind2, "" );
					g_bindings[id].bind1 = -1;
					g_bindings[id].bind2 = -1;
				} else {
					// kw: clear custom binds
					int b1, b2;
					DC->getKeysForBinding( item->cvar, &b1, &b2 );
					if( b1 != -1 )
						DC->setBinding( b1, "" );
					if( b2 != -1 )
						DC->setBinding( b2, "" );
	...
	if (id != -1) {
		...
		if (key == -1) {
			// find the command and delete the bind for it
			if (b1 != -1 )
				DC->setBinding( b1, "" );
			if (b2 != -1 )
				DC->setBinding( b2, "" );
...
void Menu_SetFeederSelection(menuDef_t *menu, int feeder, int index, const char *name) {
	Menu_SetFeederSelectionExt(menu, feeder, index, name, 0);
}


void Menu_SetFeederSelectionExt(
	menuDef_t *menu,
	int feeder,
	int index,
	const char *name,
...
void Menu_New(int handle) {
	...
	if (menuCount < MAX_MENUS) {
		...
		Com_Printf( "^3Warning: can't load menu, already maximum number of menu's loaded (%i)\n", menuCount);
	}
}
