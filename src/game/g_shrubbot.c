 ...
 *
 * File    : g_shrubbot.c
 * Desc    : Shubbot options handling code.
 *
...
qboolean _shrubbot_immutable(gentity_t *admin, gentity_t *victim)
{
    ...
    if (victim == admin)
    {
        return qfalse;                   // if admin wants to hurt himself, let him
...
void _shrubbot_writeconfig()
{
    ...
    for (i = 0; g_shrubbot_admins[i]; i++)
    {
        ...
        if (g_shrubbot_admins[i]->level == 0)
        {
            continue;
        }
...
qboolean G_shrubbot_warn(gentity_t *ent, int skiparg)
{
        ...
        (g_warningOptions.integer & WARNOP_LINK_IP))
    {
        ...
        if (store == qtrue)
        {
            ...
            for (i = 0; *ip; ip++)
            {
                if (i >= sizeof(tmp) || *vic->client->sess.ip == ':')
                {
                    break;
                }
                tmp[i++] = *ip;
...
qboolean G_shrubbot_userinfo(gentity_t *ent, int skiparg)
{
    ...
    SPC(va("^/Slot Number: ^d%i", vic - g_entities));
