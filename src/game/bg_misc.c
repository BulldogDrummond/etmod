...
#define WEAP_AMMO_RESTORES_HELMET (cgs.weapons & WPF_AMMO_RESTORES_HELMET)
#elif GAMEDLL
	extern	vmCvar_t g_developer;
	extern	vmCvar_t g_gametype;
#define gametypeCvar g_gametype
  extern vmCvar_t g_weapons;
#define WEAP_AMMO_RESTORES_HELMET \
    (g_weapons.integer & WPF_AMMO_RESTORES_HELMET)
#else
	extern	vmCvar_t ui_gameType;
#define WEAP_AMMO_RESTORES_HELMET 0 // josh: no ui stuff here
...
int weapBanksMultiPlayer[MAX_WEAP_BANKS_MP][MAX_WEAPS_IN_BANK_MP] = {
	...
	{WP_STEN,				WP_GARAND,				WP_PANZERFAUST,		WP_FLAMETHROWER,			WP_KAR98,				WP_CARBINE,					WP_FG42,	WP_K43,		WP_MOBILE_MG42,	WP_MORTAR,	WP_MP40,	WP_THOMPSON	},
...
qboolean BG_AddMagicAmmo( playerState_t *ps, int *skill, int teamNum, int numOfClips ) {
	...
	if(WEAP_AMMO_RESTORES_HELMET && (ps->eFlags & EF_HEADSHOT)) {
		if(!numOfClips)
			return qtrue;
	...
	if( COM_BitCheck( ps->weapons, WP_MEDIC_ADRENALINE ) ) {
		// forty - was maxammo (10) but you spawn with 12.
		i = skill[ SK_FIRST_AID ] >= 2 ? 12 : 10;
...
const voteType_t voteToggles[] =
	{
		...
		{ "vote_allow_shufflenorestart",	CV_SVF_SHUFFLENORESTART },
		{ "vote_allow_putspec",				CV_SVF_PUTSPEC },
		// pheno: cointoss
...
pheno: moved this from cg_consolecmds.c/g_cmds.c
==================
*/
