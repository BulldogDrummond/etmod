 ...
 * Project : ETMod
 *
 * File    : g_player_rating.c
 * Desc    : Player and kill rating code.
 *
 * Status  : Pending
 *
 *********************************************************/
...
float G_GetWinProbability(team_t team, gentity_t *ent, qboolean updateWeights)
{
    ...
    if (updateWeights && g_etmod_stats_id.integer != -1)
    {
