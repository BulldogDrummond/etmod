...
// added ignore ent so the bot wont count itself.
...
int AddBot(const MessageHelper &_data)
{
    ...
    // find a usable slot. this should avoid any game / engine sync problems related to CS_FREE
    ...
    {
        G_Printf("%sCould not connect bot: %s\n", S_COLOR_RED, s);
        num = -1;
    }
...
obResult ChangeClass(int _client, int _newclass, const MessageHelper *_data)
{
    ...
    if (_newclass <= ET_CLASS_NULL || _newclass >= ET_CLASS_MAX)
    {
            ...
            (bot->client->sess.latchPlayerType >= NUM_PLAYER_CLASSES))
        {
            ...
            // make sure one of each if min is greater than zero
    ...
    // wait until 2 seconds before next spawn
...
void UpdateBotInput(int _client, const ClientInput &_input)
{
    ...
    if (cmd.weapon == WP_GPG40 && bot->client->ps.weapon == WP_GPG40)
    {
    ...
    else if (cmd.weapon == WP_M7 && bot->client->ps.weapon == WP_M7)
    {
...
void GetGoals()
{
    ...
    for (int i = MAX_CLIENTS; i < level.num_entities; ++i)
    {
        ...
        case ET_EXPLOSIVE:
        {
            // hack - weaponclass not set yet, using xpbonus
    ...
    for (int i = 0 ; i < numofmg42s ; ++i)
    {
        ...
        // this was done in et_goalmanager before
...
obResult InterfaceSendMessage(const MessageHelper &_data, const GameEntity _ent)
{
    ...
    case ET_MSG_GOTOLIMBO:
    {
        ...
        if (pMsg)
        {
            // don't forget to look if the pEnt SHOULD really be in limbo
    ...
    case ET_MSG_PICKWEAPON2:
    {
        ...
        if (pMsg)
        {
            if (pEnt && pEnt->inuse && pEnt->client)
            {
                ...
                {
                    // /kill 2 seconds before next spawn
...
void Bot_Event_RemoveWeapon(int _client, int _weaponId)
{
    if (IsOmnibotLoaded())
    {
        ...
        // don't send the event if the bot is dead so b.HasWeapon() will work in Limbo
