...
// print out their kill rating information
...
qboolean G_UniformSteal(gentity_t *ent, gentity_t *traceEnt)
{
    ...
    if (BODY_TEAM(traceEnt) > 2)
    {
        return qfalse;                         //Perro: prevents stealing bug
    }
...
// Dens: etmodclient >= 20070819 lets the server make the reward message, so
...
//        Cmd_ClearWayPoint_f(ent);
    ...
    else if (!Q_stricmp(cmd, "etmod_version"))
    {
        etmod_version(ent);
    }
    else
